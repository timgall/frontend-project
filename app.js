...
searchBtn.on("click", () => {
  ...
  $.get(
    `https://api.openbrewerydb.org/v1/breweries?by_postal=${query}&per_page=20`,
    (data) => {
      result.html("");
      const breweries = data
        .filter((brewery) => brewery.postal_code.substr(0, 5) === query)
      ...
      if (breweries.length === 0) {
        ...
        const image = $("<img>")
          .attr(
            "src",
            "https://media.tenor.com/i1MnpOuskr4AAAAC/parks-and-rec-ron-swanson.gif"
          )
          .attr("width", "500")
          .attr("height", "300");
        breweries.forEach((name) => {
          ...
          li.on("click", () => {
            ...
            const clickedBreweryData = data.filter(
              (brewery) => brewery.name === clickedBreweryName
            )[0];
            const breweryDiv = $("<div>");
            breweryDiv.html(
              clickedBreweryData.name +
                "<br>" +
                "<br>" +
                "Brewery Type: " +
                clickedBreweryData.brewery_type +
                "<br>" +
                "<br>" +
                clickedBreweryData.address_1 +
                "<br>" +
                "<br>" +
                ...
                ", " +
                clickedBreweryData.postal_code +
                "<br>" +
                "<br>" +
                "Phone Number: " +
                clickedBreweryData.phone +
                "<br>" +
                "<br>" +
                "Website: <a href='" +
                clickedBreweryData.website_url +
                "' target='_blank'>" +
                clickedBreweryData.website_url +
                "</a>" +
                "<br>" +
...
stateSearchBtn.on("click", () => {
  ...
  $(".homePage").css("display", "none");
  $.get(
    `https://api.openbrewerydb.org/v1/breweries?by_state=${query}&per_page=20`,
    (data) => {
      stateResult.html("");
      const breweries = data
        .filter((brewery) => brewery.state_province === query)
        .map((brewery) => brewery.name);
      if (breweries.length === 0) {
...
citySearchBtn.on("click", () => {
  ...
  $(".homePage").css("display", "none");
  $.get(
    `https://api.openbrewerydb.org/v1/breweries?by_city=${query}&per_page=20`,
    (data) => {
      cityResult.html("");
      const breweries = data
        .filter((brewery) => brewery.city === query)
        .map((brewery) => brewery.name);
      if (breweries.length === 0) {
        const img = $("<img>").attr("src", "sadRon.png");
        cityResult.html("");
